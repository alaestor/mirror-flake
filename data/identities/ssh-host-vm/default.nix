let
  ...
  /**
    ...
    guest's sshd host key. The guest holds no identity of its own and
    nothing should ever encrypt *to* one of these keys — the private half is
    a runtime secret decrypted by the host that runs the guest, not a
    recovery artifact.
